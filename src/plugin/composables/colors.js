 ...
 * Checks
 */
function checkDoNotConvert(color) {
	return color === 'transparent' || color === 'none' || color === 'inherit' || color === 'currentColor' || color === 'initial' || color === 'unset';
}

function checkIfThemeVarColor(color) {
	return color.includes('--v-theme');
}

function checkIfThemeColor(color, theme) {
	const themeColors = theme.isDark ? theme.themes.dark : theme.themes.light;
...
function convertToHSL(color) {
	let newColor = checkColorNames(color);
	let h = 0;
	let s = 0;
	let l = 0;
	let r = 0;
	...
	// Convert RGB to array values if necessary
		...
		newColor = [...newColor.matchAll(/\d+/g)].map(Number);
	}
	...
	if (max === null || !min === null || isNaN(max) || isNaN(min)) {
		...
		console.warn(`[VResizeDrawer]: The "color" prop value using "${newColor}" doesn't exist. Using the value "hsl(${defaultColor})" in it's place.`);
		return defaultColor;
	}
 ...
 */
function checkColorNames(color) {
	const colors = {
		AliceBlue: '#F0F8FF',
		AntiqueWhite: '#FAEBD7',
		Aqua: '#00FFFF',
 ...
 */
export const useGetColor = (color, theme) => {
